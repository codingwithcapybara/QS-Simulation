...
const PredictiveMaintenance = () => {
  const { scoopHistory, totalScoops, totalTimeHours, soilVolume } =
    useSensorStore();
  ...
  const avgBucketCapacity =
    recentScoops.length > 0
      ? recentScoops.reduce((sum, scoop) => sum + scoop.volume, 0) /
        recentScoops.length
      : 0;

  const bucketCapacityPercentage =
    (avgBucketCapacity / BUCKET_FULL_CAPACITY) * 100;
  const bucketNeedsMaintenance =
    avgBucketCapacity < BUCKET_FULL_CAPACITY * 0.85; // 85% threshold
  ...
  const efficiencyNeedsMaintenance =
    currentEfficiency < EXPECTED_EFFICIENCY * 0.85; // 85% threshold

  // Overall maintenance status
  const requiresMaintenance =
    bucketNeedsMaintenance || efficiencyNeedsMaintenance;
    ...
    <div
      ...
      <div
        ...
        <div
          style={{
            ...
            borderRadius: '5px',
            border: `2px solid ${
              bucketNeedsMaintenance ? '#ff4444' : '#00ff88'
            }`,
          }}>
          <h4
            style={{
        ...
        <div
          ...
          <h4
            ...
            {efficiencyNeedsMaintenance ? '⚠️' : '✓'} Metric B: Machinery
            Efficiency
          </h4>
      ...
      {recentScoops.length < 5 && (
        <div
          ...
          ℹ️ Collecting data... Need at least 5 scoops for accurate maintenance
          prediction
        </div>
      )}
    </div>
